-- CreshGames/GamesDungeonAchievements.lua
-- Rework Phase 5: the 93 Dungeon Dwellers achievements, moved here from
-- CreshCollect/DungeonAchievements.lua. Merges into the shared CG.Achievements
-- catalog the same way CreshCollect's AchievementExpansion.lua/
-- ClassAchievements.lua extend COL.Achievements -- must load after
-- GamesAchievements.lua (see CreshGames.toc).
--
-- All stat reads are same-addon now (CG.DungeonDwellersPass directly, not
-- the CC bridge proxy the original CreshCollect file needed).
local _, CG = ...
if not CG then return end
if not CG.Achievements then return end
local A = CG.Achievements
...
-- Same reward formula as the original CreshCollect/DungeonAchievements.lua
-- (unchanged, so all 93 ported entries pay exactly what they always did).
...
local function addDDSeries(stat, goals, keys, titles, descriptions, weight)
    for i, goal in ipairs(goals) do
        local coins, xp = ddRewardFor(i, weight)
        A:Add("DUNGEON_DWELLERS", stat, goal, titles[i], descriptions[i], i, weight, keys[i], coins, xp)
    end
end
...
function A:BuildCatalog()
    ...
    if self.dungeonBuilt then return end
    self.dungeonBuilt = true

    -- KILLS — enemy kills inside Dungeon Dwellers
    addDDSeries("DD_KILLS",
    ...
    -- PASS LEVEL — Delver Mastery rank progression
    addDDSeries("DD_PASS_LEVEL",
        { 5, 10, 25, 50, 75, 100 },
        ...
        { "Reach Delver Mastery level 5.",
          "Reach Delver Mastery level 10.",
          "Reach Delver Mastery level 25.",
          "Reach Delver Mastery level 50.",
          "Reach Delver Mastery level 75.",
          "Reach Delver Mastery level 100." }, 2)

    -- ARMOUR — class or armour set unlocks
    addDDSeries("DD_ARMOUR",
        { 1, 3, 5, 8, 12 },
...
-- ── Dungeon-specific stat reader ──────────────────────────────────────────────
-- Same-addon reads (CG.DungeonDwellersPass directly) plus this catalog's own
-- activity table for the Pass-6-tracked stats (damage/crits/streak/crates).

local function ddActivitySave()
    if not _G.CreshGamesDB then return nil end
    _G.CreshGamesDB.dungeonAchievements = type(_G.CreshGamesDB.dungeonAchievements) == "table" and _G.CreshGamesDB.dungeonAchievements or {}
    local save = _G.CreshGamesDB.dungeonAchievements
...
-- CG.DungeonDwellersPass:Ensure() returns two values (save, dungeonRoot);
-- `and/or` truncates multi-returns to one value, so this must be a proper
-- if-statement rather than a one-line and/or expression.
local function dungeonRoot()
    if not CG.DungeonDwellersPass then return {} end
...
function A:GetStat(stat)
    if stat == "DD_KILLS" or stat == "DD_BOSSES" or stat == "DD_UNIQUE_BOSSES" or stat == "DD_RUNS"
        or stat == "DD_BEST_ROOM" or stat == "DD_HIGH_SCORE" or stat == "DD_PASS_LEVEL"
        or stat == "DD_ARMOUR" or stat == "DD_MINION" then
        local dungeon = dungeonRoot()
        ...
        if stat == "DD_BEST_ROOM" then return floor(max(0, tonumber(dungeon.bestRoom) or tonumber(dungeon.bestLevel) or 0)) end
        ...
        if stat == "DD_PASS_LEVEL" then
            ...
            local xp = floor(max(0, tonumber(dungeon.xp) or tonumber(bp.xp) or 0))
            if CG.DungeonDwellersPass.GetLevelFromXP then return CG.DungeonDwellersPass:GetLevelFromXP(xp) end
            return 1
        end
    ...
    if stat == "DD_ITEM" then
        return countMap(dungeonRoot().discoveredItems)
    end
    if stat == "DD_CRATE" or stat == "DD_DAMAGE_DEALT" or stat == "DD_DAMAGE_TAKEN"
        or stat == "DD_CRITS" or stat == "DD_STREAK" then
        local save = ddActivitySave()
        local act = save and save.activity or {}
        if stat == "DD_CRATE" then return floor(max(0, tonumber(act.cratesOpened) or 0)) end
        if stat == "DD_DAMAGE_DEALT" then return floor(max(0, tonumber(act.damageDealt) or 0)) end
        if stat == "DD_DAMAGE_TAKEN" then return floor(max(0, tonumber(act.damageTaken) or 0)) end
        if stat == "DD_CRITS" then return floor(max(0, tonumber(act.crits) or 0)) end
        if stat == "DD_STREAK" then return floor(max(0, tonumber(act.bestStreak) or 0)) end
    end
    return oldGetStat(self, stat)
end

-- ── Dungeon-specific unlock ───────────────────────────────────────────────────
-- Dungeon Dweller achievements pay their XP into Delver Mastery (their own
-- Pass), not the generic Arcade Pass "achievement completion" hook the base
-- Unlock uses for the other (ex-GAMES) achievements -- this is unchanged
-- behavior from the original CreshCollect/DungeonAchievements.lua, just
-- same-addon now instead of routed through the guarded CreshGamesAPI.
local oldUnlock = A.Unlock
function A:Unlock(achievement, silent)
    if not achievement or achievement.category ~= "DUNGEON_DWELLERS" then
        return oldUnlock(self, achievement, silent)
    end
    local save = self:Ensure()
    if not save or save.unlocked[achievement.key] then return false end
    ...
    if CG.BattlePass and CG.BattlePass.AddCoins then CG.BattlePass:AddCoins(achievement.coins, "DD_ACHIEVEMENT") end
    if CG.DungeonDwellersPass and CG.DungeonDwellersPass.AddXP then
        CG.DungeonDwellersPass:AddXP(achievement.xp, "DD Achievement: " .. achievement.title, nil, silent)
    end
...
-- ── Recording API (stubs; a future pass can wire DungeonCrawlerContent.lua
-- to call these at the appropriate game events, same as the original) ───────

function A:RecordKill()
    self:EvaluateAll(false)
end

function A:RecordBoss()
    self:EvaluateAll(false)
end

function A:RecordRunComplete()
    self:EvaluateAll(false)
end

function A:RecordDamageDealt(amount)
    local save = ddActivitySave()
    if not save then return end
    save.activity.damageDealt = (save.activity.damageDealt or 0) + floor(max(0, tonumber(amount) or 0))
end

function A:RecordDamageTaken(amount)
    local save = ddActivitySave()
    if not save then return end
    save.activity.damageTaken = (save.activity.damageTaken or 0) + floor(max(0, tonumber(amount) or 0))
end

function A:RecordCrit()
    local save = ddActivitySave()
    if not save then return end
    save.activity.crits = (save.activity.crits or 0) + 1
end

function A:RecordCrateOpen()
    local save = ddActivitySave()
    if not save then return end
...
function A:RecordStreakUpdate(streak)
    local save = ddActivitySave()
    if not save then return end
