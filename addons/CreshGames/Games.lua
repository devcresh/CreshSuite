...
CG.Games = Games
if CG.RegisterModule then CG:RegisterModule("Games", Games) end
...
function Games:SendLeaderboard(target, force)
    ...
    if target == "" or not CG.SoloGames or not CG.SoloGames.GetLeaderboardSnapshot then return false end
    ...
    local snapshot = CG.SoloGames:GetLeaderboardSnapshot()
...
function Games:RecordResult(result, detail, roundKey)
    ...
    if CG.SoloGames and CG.SoloGames.RecordHistory then
        CG.SoloGames:RecordHistory(active.game, "MULTIPLAYER", result, active.opponent, detail)
    end
    if CreshGamesDB then
        CreshGamesDB.multiplayerStats = CreshGamesDB.multiplayerStats or {}
        local stats = CreshGamesDB.multiplayerStats[active.game] or { wins = 0, losses = 0, draws = 0, games = 0 }
...
function Games:GetTargets()
    ...
    local function add(name, online, source)
        name = cleanName(name)
        if name == "" or CG:IsSelf(name) then return end
...
function Games:Challenge(target, game)
    ...
    if target == "" then if CC.Print then CC:Print("Choose an online friend or whisper contact first.") end; return false end
    if self.active then if CC.Print then CC:Print("Finish or close the current game before starting another challenge.") end; return false end
    if self.pendingOutgoing then if CC.Print then CC:Print("A game challenge is already waiting for a response.") end; return false end
    ...
    if not self:Send(target, "C", id, game, self.protocol) then
        ...
        if CC.Print then CC:Print("Unable to send that game challenge.") end
        self:RefreshHub()
        return false
    end
...
function Games:ShowChallengePopup(sender, game, id)
    ...
    if not self.challengePopup then
        local popup = CreateFrame("Frame", "CreshGamesGameChallenge", UIParent, templateName())
...
function Games:HandleAddonMessage(prefix, payload, channel, sender)
    ...
    if sender == "" or CG:IsSelf(sender) then return end
    ...
    elseif kind == "L" then
        self:MarkPeer(sender, "CreshChat")
        if CG.SoloGames and CG.SoloGames.ReceiveLeaderboard then CG.SoloGames:ReceiveLeaderboard(sender, parts) end
    elseif kind == "C" then
...
function Games:BuildGameWindow()
    ...
    local colors = palette()
    local frame = CreateFrame("Frame", "CreshGamesGameWindow", UIParent, templateName())
    ...
    frame:SetScript("OnHide", function(selfFrame) selfFrame:EnableKeyboard(false); if CG.GameAudio and CG.GameAudio.StopMusic then CG.GameAudio:StopMusic() end end)
    frame:SetScript("OnKeyDown", function(_, key)
        ...
        if view and CG.GameAudio and CG.GameAudio.PlayInteraction then
            CG.GameAudio:PlayInteraction(active.game == "HOLDEM" and "CARD" or "MOVE")
        end
...
function Games:BuildTETRISView()
    ...
    function view:SetDuration(minutes, broadcast)
        ...
        local save = CG.Tetris and CG.Tetris:Ensure() or nil
    ...
    function view:SendSnapshot()
        ...
        local theme = CG.Tetris and CG.Tetris:GetSelectedTheme() or nil
        local revealBackground, revealLines, _, isRevealing = nil, 100, 1, false
        if CG.Tetris and CG.Tetris.GetRevealProgress then
            revealBackground, revealLines, _, _, _, _, isRevealing = CG.Tetris:GetRevealProgress()
        elseif CG.Tetris and CG.Tetris.GetRevealTheme then
            revealBackground, revealLines, _, isRevealing = CG.Tetris:GetRevealTheme()
        end
    ...
    function view:Refresh()
        ...
        local localTheme = CG.Tetris and CG.Tetris:GetSelectedTheme() or nil
        local localBackground, revealLines, localRevealStage, linesToNextPart, linesToUnlock, _, isRevealing = nil, 100, 10, 0, 0, 1, false
        if CG.Tetris and CG.Tetris.GetRevealProgress then
            localBackground, revealLines, localRevealStage, linesToNextPart, linesToUnlock, _, isRevealing = CG.Tetris:GetRevealProgress()
        elseif CG.Tetris and CG.Tetris.GetRevealTheme then
            localBackground, revealLines, _, isRevealing = CG.Tetris:GetRevealTheme()
        ...
        local remoteTheme = CG.Tetris and CG.Tetris:GetTheme(self.remoteThemeKey or "CLASSIC_BLOCKS") or localTheme
        local remoteBoardBackground = nil
        if CG.Tetris and self.remoteRevealThemeKey and self.remoteRevealThemeKey ~= "" and CG.Tetris.GetBackground then
            remoteBoardBackground = CG.Tetris:GetBackground(self.remoteRevealThemeKey)
        end
    ...
    function view:Start(seed, mode, fromNetwork, durationMinutes)
        ...
        local tetrisSave = CG.Tetris and CG.Tetris:Ensure() or nil
        ...
        self.dropElapsed, self.statElapsed, self.dropInterval = 0, 0, (CG.Tetris and CG.Tetris:GetDropInterval(0) or 0.90)
        self.nextPiece = tetrisRandom(self)
...
function Games:BuildHOLDEMView()
    ...
    function view:SetCard(frameCard, card, hidden)
        if hidden then
            ...
            if CG.CardDecks and CG.CardDecks.ApplyCardFrame and CG.CardDecks:ApplyCardFrame(frameCard, card, true, "HOLDEM") then return end
            ...
            if CG.CardDecks and CG.CardDecks.ApplyCardFrame and CG.CardDecks:ApplyCardFrame(frameCard, card, false, "HOLDEM") then return end
            if frameCard.cardTexture then frameCard.cardTexture:Hide() end
...
-- Addon event bridge -----------------------------------------------------------
local eventFrame = CreateFrame("Frame", "CreshGamesGamesEventFrame")
...
eventFrame:SetScript("OnEvent", function(_, event, ...)
    if event == "ADDON_LOADED" then
        local loaded = ...
        if loaded == "CreshGames" or loaded == "CreshChat" then Games:RegisterPrefix() end
    ...
    elseif event == "CHAT_MSG_ADDON" then
        if CC.IsFeatureEnabled and CC:IsFeatureEnabled("multiplayerGames") then Games:HandleAddonMessage(...) end
