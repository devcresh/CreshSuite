...
COL.DungeonAchievements = DungeonAchievements
if COL.RegisterModule then COL:RegisterModule("DungeonAchievements", DungeonAchievements) end
...
-- Returns the Dungeon Dwellers game state from CreshGames via the CC bridge.
-- Nil when CreshGames is not loaded or the save is not yet initialised.
local function ddGameSave()
    local ddp = CC.DungeonDwellersPass
    if ddp and ddp.Ensure then return ddp:Ensure() end
    return nil
end

-- Returns the DD achievement tracking table stored in CreshCollectDB.
local function ddAchSave()
    if not CreshCollectDB then return nil end
    CreshCollectDB.ddAchievements = type(CreshCollectDB.ddAchievements) == "table" and CreshCollectDB.ddAchievements or {}
    local save = CreshCollectDB.ddAchievements
...
function DungeonAchievements:Unlock(achievement, silent)
    ...
    if COL.BattlePass and COL.BattlePass.AddCoins then
        COL.BattlePass:AddCoins(achievement.coins, "DD_ACHIEVEMENT")
    end
...
function DungeonAchievements:MigrateFromWoW()
    ...
    local wowAch = CreshCollectDB and type(CreshCollectDB.achievements) == "table" and CreshCollectDB.achievements or nil
...
eventFrame:SetScript("OnEvent", function(_, event)
    if event == "PLAYER_LOGIN" then
        ...
        if CC.IsFeatureEnabled and CC:IsFeatureEnabled("gameProgression") then
