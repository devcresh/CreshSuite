...
--
-- Stats stored in CreshCollectDB.gameProgression.achievements.stats:
...
local CombatTracker = { version = COL.version }
COL.CombatTracker = CombatTracker
if COL.RegisterModule then COL:RegisterModule("CombatTracker", CombatTracker) end
...
local function tryEval(force)
    ...
    dirty    = false
    lastEval = t
    if COL.Achievements and COL.Achievements.EvaluateAll then
        COL.Achievements:EvaluateAll(true)  -- silent=true; no toast spam during combat
    end
end
...
local function onCombatLog()
    if CC.IsFeatureEnabled and not CC:IsFeatureEnabled("combatTracking") then return end
...
function CombatTracker:Init()
    ...
    local ach = COL.Achievements and COL.Achievements:Ensure()
